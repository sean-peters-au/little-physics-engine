...
void Renderer::renderParticles(const entt::registry &registry, ColorMapper colorMapper)
{
    ...
    for (auto entity : view)
    {
        ...
        {
            // We have a polygon; let's draw it using sf::ConvexShape
            ...
            for (size_t i = 0; i < poly->vertices.size(); ++i)
            {
                ...
                double rx = v.x * cosA - v.y * sinA;
                double ry = v.x * sinA + v.y * cosA;

                double worldX = pos.x + rx;
                double worldY = pos.y + ry;

                float vx_px = static_cast<float>(SimulatorConstants::metersToPixels(worldX));
                float vy_px = static_cast<float>(SimulatorConstants::metersToPixels(worldY));
        ...
        {
            // Fallback: circle or square (legacy)
                        ...
                        SimulatorConstants::SimulationType highlightedScenario)
{
    ...
    const Components::SimulatorState *statePtr = nullptr;
    if (!sv.empty()) {
        statePtr = &registry.get<Components::SimulatorState>(sv.front());
    }
    ...
    for (const auto &speed : speeds)
    {
        ...
        if (statePtr) {
            if (std::abs(statePtr->timeScale - speed.first) < 0.01) {
